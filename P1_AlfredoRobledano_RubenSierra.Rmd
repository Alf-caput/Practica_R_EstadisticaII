...
```
También podría ser interesante mirar la influencia entre los problemas cardiacos y los infartos al corazón y el IMC:
```{r}
boxplot(bmi ~ heart_disease, data = df, ylab = "IMC", xlab = "Cardiopatías")
boxplot(bmi ~ stroke, data = df, ylab = "IMC", xlab = "Infartos al corazón")
```
...
```{r}
spineplot(table(df$hypertension,df$gender),xaxlabels="Hipertensión")
spineplot(table(df$stroke,df$gender),xaxlabels = "Ataques cardiacos")
spineplot(table(df$heart_disease,df$gender),xaxlabels="Cardiopatías")
```